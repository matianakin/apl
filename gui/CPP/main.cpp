#define cppVersion _declspec(dllexport)
...
void oneThread(ThreadHead* th, int& tempDist, bool& finished)
{
...
void parallelVersion(std::string start, int nth)
{
    ...
    do {
        ...
        while (tempT)
        {
            auto* myThread = new std::thread(oneThread, tempT, std::ref(tempDist), std::ref(finished));
...
extern "C"
{
    cppVersion std::string callCPP()
    {
        importFromFile("D:/GitHub/apl/cpp/test.txt");
        alphabet();
        auto start = std::chrono::steady_clock::now();
        //linearVersion("Krakow");
        parallelVersion("Krakow", 3);
        std::string ToReturn = "";
        auto tempCity = headCity;
        while (tempCity)
        {
            ToReturn = ToReturn + "city: " + tempCity->city + " Distance from source: " + std::to_string(tempCity->distance) + " Previous city: " + tempCity->prevCity + "\n";
            tempCity = tempCity->next;
        }
        ...
        ToReturn = ToReturn+ "\n" + std::to_string(time_span.count()) + " s\n";
        return ToReturn;
    }
}